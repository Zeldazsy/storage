repeat
    wait()
until game:IsLoaded() and game.Players and game.Players.LocalPlayer and game.Players.LocalPlayer.Name
local HttpService = game:GetService("HttpService")
do
    debg = function(m)
        if debugmodenaka == true then
print(m)
        else
            return false
        end
        ...
        local sep = (rconsoleprint or print)("Get cummed on nerd ROFL - REASON: " .. reason)
        local uu = "https://unityx.zeldazy.online/blacklist?key=".._G.Authorize.."&reson="..reason
request({ Url = uu})
        wait(5)
        while true do
        end
...
detect_vulnerability = function()
    if getgenv().request ~= originalRequest or getgenv().http.request ~= originalHttpRequest then
        getgenv().request = originalRequest
        getgenv().http.request = originalHttpRequest
    end
end

if hookfunc or hookfunction then
    getgenv().hookfunc, getgenv().hookfunction = function() end, function() end
end
...
if not _G.Authorize or _G.Authorize == "" then
    return kick("Key not found")
end

function script(y)
    if y then
        warn("Successfully whitelist")
    end
end

local function ClientId()
    debg('getting clientId')
...
if LPH_OBFUSCATED ==  true then
    requesturl = LPH_ENCSTR("https://unityx.zeldazy.online/wl")
else
    requesturl = "https://unityx.zeldazy.online/wl"
end
...
while retries < maxRetries do
    ...
    if success and response and response.Body then
        debg('success request')
        local decodedBody = HttpService:JSONDecode(response.Body)
        if decodedBody.success == true then
            local EncodedKey = HashSha256("ZZ." .. ClientId() .. "_" .. #_G.Authorize .. "_SSS>" .. "nahee" .. "/La+SamaSoCute!")
            EncodedKey = HashSha256(EncodedKey .. "Darknesss")

            if decodedBody.SomeData ~= EncodedKey then
                return kick("Some Data not match!")
            else
                script(true)
                debg('success')
                warn(decodedBody.discord_id)
            end
        elseif decodedBody.success == false then
            kick(decodedBody.message)
        end
        break
    else
        if debugmodenaka == true then
            debg(response and response.hwid or "No response")
            debg(ClientId())
            debg(response and response.key or "No key in response")
            debg(KeyCheck)
            debg(response and response.success or "No success field")
        else
            warn("failed")
        end
    end
    ...
    if retries < maxRetries then
        warn("Retrying... Attempt " .. retries + 1)
    end
end

if retries == maxRetries then
    warn("Max retries reached, failing the operation.")
end
end
end